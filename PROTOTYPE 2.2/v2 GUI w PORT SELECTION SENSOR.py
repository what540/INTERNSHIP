...
ultrasonic_values = deque(maxlen=2)
laser_values = deque(maxlen=2)
ultrasonic_previous_values = None
laser_previous_values = None
ultrasonic_last_update_times = 0
laser_last_update_times = 0
...
def ultrasonic_update_sensor_data(distance):
    ...
    global ultrasonic_last_update_times,ultrasonic_previous_values

    current_time = time.time()

    # Update range difference every second
    root.after(0, ultrasonic_update_distance_label, distance)
    ultrasonic_values.append(distance)
    ...
    if current_time - ultrasonic_last_update_times >= 1:  # after 1s
        root.after(0, ultrasonic_calculate_and_update_range, ultrasonic_previous_values, distance)
        ultrasonic_last_update_times = current_time
        ultrasonic_values.clear()
    ultrasonic_previous_values = distance


def laser_update_sensor_data(distance):
    """Update sensor data and GUI labels."""
    global laser_last_update_times,laser_previous_values

    current_time = time.time()

    # Update range difference every second
    root.after(0, laser_update_distance_label, distance)
    laser_values.append(distance)
    ...
    if current_time - laser_last_update_times >= 1:  # after 1s
        root.after(0, laser_calculate_and_update_range, laser_previous_values, distance)
        laser_last_update_times = current_time
        laser_values.clear()

    laser_previous_values = distance
...
def ultrasonic_calculate_and_update_range(previous_value, current_value):
...
def laser_calculate_and_update_range(previous_value, current_value):
    """Calculate and update the range difference."""
    range_value = current_value - previous_value if previous_value is not None else 0
