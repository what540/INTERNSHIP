...
char tx_buffer[50];  // Buffer for data to send
void RS485_SetTX() {
...
void setup() {
  ...
  Serial2.begin(115200, SERIAL_8N1, RX_PIN, TX_PIN);  // Initialize RS485 Control Pin
  pinMode(DE_RE_PIN, OUTPUT);
  RS485_SetRX();  // Start in Receive mode
    ...
    if (currentIndex >= numReadings) {
      if (currentMillis - previousMillis >= interval) {
        ...
        snprintf(tx_buffer, sizeof(tx_buffer),
                 "Ultrasonic: %.4f, Laser: %.4f, Diff Ultra: %.4f, Diff Laser: %.4f, Prev Ultra: %.4f, Prev Laser: %.4f, Gyro: %.2f",
        ...
        Serial2.printf("Data sent: %s\n", tx_buffer);  // Print debug message
